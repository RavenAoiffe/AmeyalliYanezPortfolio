const Header = () => {
return(
<nav id="menu">
  <p className="fechas">
    <strong>Ameyalli </strong>
    Yáñez
  </p>
  <div className="menu-rs">
    <div className="flex">
      <a
        style={{ display: "none" }}
        href="https://www.tiktok.com/@ravenyanez"
        rel="noopener"
        target="_blank"
        className="mas"
      >
        <div className="tik-tok" />
      </a>
      <a
        href="https://github.com/RavenAoiffe"
        rel="noopener"
        target="_blank"
        className="mas"
      >
        <div className="github" />
      </a>
      <a href="#" style={{ display: "none" }} className="mas noche">
        <img src="data:image/png;base64," />
      </a>
    </div>
    <div id="menu_movil" className="menu_movil">
      <span />
      <span />
      <span />
    </div>
  </div>
  <ul id="navegacion" className="navegacion-principal">
    <li>
      <a href="#" className="cerrar">
        X
      </a>
    </li>
    <li>
      <a className="enlace" href="#presentacion">
        Sobre mi
      </a>
    </li>
    <li>
      <a className="enlace" href="#Porta">
        Proyectos
      </a>
    </li>
    <li>
      <a className="enlace" href="#Experiencia">
        Experiencia
      </a>
    </li>
    <li>
      <a className="enlace" href="#Educacion">
        Educación
      </a>
    </li>
    <li>
      <a className="enlace" href="#Habilidades">
        Habilidades
      </a>
    </li>
    <li>
      <a className="enlace" href="#Cursos">
        Cursos
      </a>
    </li>
    <li>
      <a href="#publicaciones" className="enlace">
        Publicaciones
      </a>
    </li>
    <li>
      <a className="enlace" href="#EnQue">
        ¿En qué estoy ahora?
      </a>
    </li>
    <li>
      <a className="enlace" href="#Contacto">
        Contacto
      </a>
    </li>
    <a href="https://ravenaoiffe.github.io/AmeyalliYanez/en/">ENGLISH</a>
  </ul>
</nav>

)
}

export default Header;
